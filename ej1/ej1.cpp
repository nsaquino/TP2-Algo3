...
int main(int argc, char *argv[]){
    ...
    //P(ganar) = (m/n) * ((m-1)/(n-1)) + ((n-m)/n) * ((n-m-1)/(n-1))    ???
    //P(ganar) = ( comb(ver_cc,2)/ver_cc[i] ) * P(elegir cc)
    //                                         1/cant_cc
    ...
    float res = 1.0 - ((1.0*cant_formas_de_ganar)/(1.0*cant_formas_de_jugar)); //((float)(res/(float) cant_formas_de_jugar));
...
/*
    1° calc puentes
    2° para cada COMP hacer dfs y calcular cantidad, con eso calcular fórmula y listo.
*/